...
import { atLeastOneSelected, isFieldInvalid, uniqueItemsValidator, validateMaxPrice } from '../../../../../core/validators/forms.validator';
...
export class AddEventCompanyComponent {
  ...
  constructor(
    ...
    this.EventPlannerRegistrationForm = this.fb.group({
      companyInfo: this.fb.group({
          ...
          minPrice: ['', [Validators.required, Validators.min(0), Validators.pattern('^[0-9]+$')]],
          maxPrice: ['', [Validators.required, validateMaxPrice, Validators.pattern('^[0-9]+$')]],
  ...
  isServiceValid(): boolean {
    return this.EventPlannerRegistrationForm.get('serviceInfo.services')?.value.some(
      (obj: service)=> obj.selected === true)
  }

  isFieldError(fieldgroup: string, fieldName: string): boolean | undefined{
    if(fieldgroup){
      return isFieldInvalid(this.EventPlannerRegistrationForm, `${fieldgroup}.${fieldName}`);
    }
    return isFieldInvalid(this.EventPlannerRegistrationForm, `${fieldName}`);
  }

  get portfolioControls(): FormArray {
    return this.EventPlannerRegistrationForm.get('portfolioInfo.portfolios') as FormArray;
  }

  // Convenience method to get the FormArray for services
  get serviceControls():  service[] {
    return (this.EventPlannerRegistrationForm.get('serviceInfo.services') as FormArray).value;
  }
  ...
  get plannedCities(): FormArray {
    return this.EventPlannerRegistrationForm.get('serviceInfo.plannedCities') as FormArray;
  }
  ...
  initializeServicesCheckboxes() {
    this.eventServicesOptions.forEach(value => {
      ...
      (this.EventPlannerRegistrationForm.get('serviceInfo.services') as FormArray).push(control);
    });
  }

  onCheckboxChange(event: Event, index: number) {
    const isChecked = (event.target as HTMLInputElement).checked;
    const servicesArray = this.EventPlannerRegistrationForm.get('serviceInfo.services') as FormArray;
    const control = servicesArray.at(index) as FormGroup;
    control.patchValue({  name: control.value.name, selected: isChecked });
  }

  onAddCity() {
    const control = this.fb.control('', [Validators.required, Validators.minLength(3)]);
  ...
  isFieldRequired(fieldgroup: string, fieldName: string): boolean {
    const control = this.EventPlannerRegistrationForm.get(`${fieldgroup}.${fieldName}`);
  ...
  onDocumentSelected(event: Event): void {
    ...
    if (file) {
      ...
      reader.onload = (e: any) => {
        ...
        this.EventPlannerRegistrationForm.get('serviceInfo.document')?.patchValue(file);
      };
  ...
  onFileSelected(event: any) {
    ...
    if (files && files.length > 0) {
      ...
      for (let i = 0; i < files.length && (files.length <= 5) && this.uploadedImages.length < 5; i++) {
      ...
      if(files.length > 5){
        this.toastr.warning('Only 5 images can be chosen', 'Warning');
      }
    }
  }
  ...
  onCoverPicSelected(event: Event): void {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        this.EventPlannerRegistrationForm.get('portfolioInfo.coverPic')?.patchValue(file);
      };
      reader.readAsDataURL(file);
    }
  }
  ...
  canProceed(): boolean {
    ...
    if (this.activeTab === 0) {
      ...
      return controls['companyInfo'].valid && controls['addressInfo'].valid && controls['portfolioInfo'].valid;
  ...
  onSubmit(){
    ...
    if(this.EventPlannerRegistrationForm.valid && this.isSubmitForm){
        ...
        const selectedServices: string[] = this.EventPlannerRegistrationForm.get('serviceInfo.services')?.value
        .filter((value: service) => value.selected)
        .map((value: service) => value.name);

        // Create FormData object
        const formData = new FormData();
        formData.append('companyInfo', JSON.stringify(controls['companyInfo']?.value));
        formData.append('addressInfo', JSON.stringify(controls['addressInfo']?.value));
        formData.append('description', sanitizedContent);
        formData.append('services', JSON.stringify(selectedServices));
        formData.append('plannedCities', JSON.stringify(controls['serviceInfo'].value['plannedCities']));
        ...
        this.eventplannerservice.submitEventPlannerForm(formData).subscribe({
          next: (res) => {
              this.toastr.success("You Service SuccessFully Register Your Service!", 'Warning');
              this.router.navigate(['/vendor/event-planner/service'], { replaceUrl: true });
          },
          error: (err: any) => {
